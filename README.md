...
* (Required) -wordsfile [WORDS_FILE]: A user-provided list of words to mash in a readable file. Words must have length {len | 2 > len < 10} to be picked up by the program. The program selects 2 or 3 distinct words for every frankenword, and so this creates a relationship between the number of words in the words file and the number of frankenwords requested. Please be mindful of this relationship to ensure program does not exit without generating any output. (The user will be warned in this event. See LOGGING and EXCEPTIONS sections below.) Words must be separated in the file by a carriage return, i.e. hitting Return between each typed word. A handy file of 58,000 English words may be downloaded [here](http://www.mieliestronk.com/wordlist.html). A copy of this file is included in the project root (english_words.txt).
